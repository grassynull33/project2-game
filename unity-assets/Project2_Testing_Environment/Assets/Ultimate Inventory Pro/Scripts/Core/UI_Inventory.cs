            ...
            if (found == false) //Checks whether the item has been placed yet or not.
            {
                if (allSlots[i] != null)
                {
                    ...
                    if (allSlots[i].GetComponent<UI_Item>().itemID == item.GetComponent<UI_Item>().itemID && allSlotsStack[i] < allSlots[i].GetComponent<UI_Item>().maxStack)
                    {
                        ...
                        eventManager.PickupItemEvent(itemsList.allItems[item.GetComponent<UI_Item>().itemID], i, true, item.GetComponent<UI_Item>().desc); //Send information about the pickup on the event manager.
                    }
                }
                ...
                else
                {
                    ...
                    Destroy(item); //Destroy the world model.
                    eventManager.PickupItemEvent(itemsList.allItems[item.GetComponent<UI_Item>().itemID], i, false, item.GetComponent<UI_Item>().desc); //Send information about the pickup on the event manager.
                }
            }
        }
